...
class SimpleBrowserlessMCPServer {
  ...
  private registerTools() {
      ...
      async ({ url, waitForSelector }) => {
        try {
          const response = await axios.post(
            `${this.browserlessUrl}/content`,
            { url, ...(waitForSelector ? { waitForSelector } : {}) },
            { params: this.params(), timeout: 30000, responseType: 'text' }
          );
          return {
            content: [
      ...
      async ({ url, options }) => {
        try {
          const response = await axios.post(
            ...
            { url, options: options || {} },
            { params: this.params(), responseType: 'arraybuffer', timeout: 60000 }
          );
    ...
    this.server.registerTool(
      'test_connection',
      { title: 'Test Connection', description: 'Test connection to the Browserless instance', inputSchema: {}, annotations: { readOnlyHint: true } },
      async () => {
        try {
